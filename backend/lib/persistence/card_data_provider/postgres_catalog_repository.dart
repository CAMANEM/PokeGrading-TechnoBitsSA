...
class PostgresCatalogRepository implements CatalogRepository {
  ...
  Future<PokemonCard> saveCard(AddPokemonCardInput input) async {
    return await _connection.runTx((tx) async {
      ...
      final cardDisplayName = input.display?.displayName?.trim().isNotEmpty ==
              true
      ...
      await tx.execute(
        ...
        parameters: [
          ...
          input.identity.number.trim(),
          input.identity.edition.trim(),
          input.identity.language.trim(),
          input.identity.finish.trim(),
          input.display?.year,
          input.display?.rarity?.trim(),
  ...
  PokemonCard _rowToCard(List<dynamic> row) {
    ...
    final display =
        CardDisplay(displayName: row[6]?.toString(), author: row[9].toString());
    return PokemonCard(
      id: row[0].toString(),
      identity: identity,
      display: display,
